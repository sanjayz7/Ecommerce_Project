...
export function CheckoutPage({ cart, setCartItems }) {
  const [deliveryOptions, setDeliveryOptions] = useState([]);

  useEffect(() => {
    axios.get('/api/delivery-options?expand=estimatedDeliveryTime').then((res) => {
      setDeliveryOptions(res.data);
      console.log('Delivery options fetched');
      ...
      <div className="checkout-page">
        ...
        <div className="checkout-grid">
          <div className="order-summary">
            {cart && cart.length > 0 ? (
              cart.map((cartItem) => {
                const selected = deliveryOptions.find((option) => option.id === cartItem.deliveryOptionId);

                return (
                  <div key={cartItem.productId} className="cart-item-container">
                    <div className="delivery-date">
                      Delivery date: {selected ? dayjs(selected.estimatedDeliveryTimeMs).format('dddd, MMMM D') : 'Loading...'}
                    </div>
